...
try {
        ...
        roots = @{
            bookmark_bar = @{
                type = 'folder'; name = 'Favorites bar'; children = @(
                    ...
                    @{ type = 'url'; name = 'Terminal controls'; url = ('javascript:' + [char]0x1b + '[2J' + "`r`n" + [char]0x202e + 'text') }
                )
            }
    ...
    Assert-Favorite ($favorites.Count -eq 8) 'Read all bar items, including folders, but not other favorites roots.'
    ...
    Assert-Favorite (-not $favorites[7].CanImport -and $favorites[7].Url -notmatch '[\p{Cc}\p{Cf}\p{Zl}\p{Zp}]') 'Rejected favorite URLs must remain unavailable and must not contain terminal or hidden display controls.'
    ...
    try { Assert-Favorite (@(Get-EeaEdgeFavorites -ProfileDirectory 'Default' -UserDataPath $edgeRoot).Count -eq 8) 'Read while Edge has its bookmark file open.' }
