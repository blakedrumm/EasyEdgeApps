...
The [second compiled run](https://github.com/blakedrumm/EasyEdgeApps/actions/runs/34511223333) completed its build/package stage, then stopped with 180 Core passes and two file-lock fixture failures. Blocking writes shared the thread pool with the continuations that release the intentional reader locks, exposing a scheduling dependency on the smaller runner. Those two test workers now use dedicated threads; all 182 Core cases passed locally with `DOTNET_PROCESSOR_COUNT=2`. Exact assertions, production retry bounds and `p4` binaries are unchanged. The [second retained Windows run](https://github.com/blakedrumm/EasyEdgeApps/actions/runs/34511223353) passed all three jobs. This scheduling correction is not a diagnosis of the older unrelated filesystem errors recorded below.

The [third compiled run](https://github.com/blakedrumm/EasyEdgeApps/actions/runs/34512294443) passed all 182 Core cases, then recorded 83 Windows passes, the explicit topmost skip and one failure deleting the synthetic pin helper's executable during fixture disposal. The fixture now retries only that owned executable's access-denied/sharing-lock deletion for at most about five seconds after cancellation and handle disposal. Its immediate helper-exit assertion is unchanged; persistent cleanup failures still fail. Both focused pin tests passed locally with the two-processor cap. The external lock owner was not identified, and no production code or release asset changed. The [third retained Windows run](https://github.com/blakedrumm/EasyEdgeApps/actions/runs/34512294380) passed all three jobs.
