...
```powershell
msiexec.exe /i "C:\Downloads\EasyEdgeApps-1.3.1-x64.msi" /qn /norestart
msiexec.exe /famus "C:\Downloads\EasyEdgeApps-1.3.1-x64.msi" /qn /norestart
msiexec.exe /x "C:\Downloads\EasyEdgeApps-1.3.1-x64.msi" /qn /norestart
```
...
```powershell
pwsh.exe -NoProfile -STA -File .\tests\Test-EasyEdgeAppsInstaller.ps1 -MsiPath .\artifacts\EasyEdgeApps-1.3.1-x64.msi
```

Use a disposable Windows account or CI runner for the actual lifecycle test:

```powershell
pwsh.exe -NoProfile -STA -File .\tests\Test-EasyEdgeAppsInstaller.ps1 -MsiPath .\artifacts\EasyEdgeApps-1.3.1-x64.msi -InstallLifecycle
```
